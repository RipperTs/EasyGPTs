...
const NodeCode = ({ data, selected }: NodeProps<FlowNodeItemType>) => {
          ...
          <Box>
            <Flex mb={2} alignItems={'center'} className="nodrag">
              <MySelect<SandboxCodeTypeEnum>
                ...
                onchange={(newLang) => {
                  const confirmFn = openSwitchLangConfirm(() => {
                    onChangeNode({
                      nodeId,
                      type: 'updateInput',
              ...
              />
              {codeType.value === 'py' && (
                <QuestionTip ml={2} label="支持import列表：pandas，numpy" />
              )}
              <Box
                cursor={'pointer'}
                color={'primary.500'}
  ...
  }, [codeType, nodeId, onChangeNode, openResetTemplateConfirm, openSwitchLangConfirm, workflowT]);
