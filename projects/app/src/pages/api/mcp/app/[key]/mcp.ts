...
export type mcpBody = { toolName?: string; inputs?: Record<string, any> };

const handlePost = async (req: ApiRequestProps<mcpBody, mcpQuery>, res: ApiResponseType<any>) => {
  const key = req.query.key;
  const server = new Server(
    { name: 'fastgpt-mcp-server-http-streamable', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
  ...
  try {
    ...
    const handleToolCall = async (
      ...
      try {
        ...
        return { message: error?.message || 'error', content: [], isError: true };
      }
    };
